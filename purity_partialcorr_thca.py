...
# ---------- ESTIMATE purity (R) ----------
logcpm.rename_axis("GeneSymbol").to_csv("estimate_input.tsv", sep="\t")
subprocess.run(["Rscript","run_estimate.R","estimate_input.tsv","estimate_scores.gct"], check=True)
sc = pd.read_csv("estimate_scores.gct", sep="\t", skiprows=2, index_col=0).drop(columns=["Description"])
...
pd.concat([cp.assign(type="checkpoint"), ic.assign(type="immune_cell")]).to_csv("PHLDA3_purity_partialcorr.csv", index=False)
