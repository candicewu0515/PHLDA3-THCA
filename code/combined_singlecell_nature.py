...
gn = summarize(pd.read_csv("data/thyroid_sc_phlda3.csv"), "cell_type", "PHLDA3", 50)
gt = summarize(pd.read_csv("data/PHLDA3_tumor_sc.csv"),  "celltype",  "raw",    30)
...
fig.savefig(f"figures/{base}.pdf", bbox_inches="tight")   # PDF only (submission vector)
plt.close(fig)
