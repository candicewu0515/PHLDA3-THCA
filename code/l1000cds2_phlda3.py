...
up = [g.strip() for g in open("data/PHLDA3_cmap_up.txt") if g.strip()]
dn = [g.strip() for g in open("data/PHLDA3_cmap_down.txt") if g.strip()]
...
df.to_csv("data/PHLDA3_L1000CDS2_hits.csv", index=False)
...
agg.to_csv("data/PHLDA3_L1000CDS2_drugs.csv", index=False)
